...
import { EquipmentType, Equipment } from '../types/fishing';
import { updateEquipment, equipmentTypeOptions } from '../utils/equipmentManager';
...
const initForm = () => {
    if (props.equipment) {
        ...
        form.type = props.equipment.type;
        form.brand = props.equipment.brand || '';
        form.description = props.equipment.description || '';
        form.images = props.equipment.images.map(url => ({ url }));
    }
};
...
const submitForm = async () => {
    ...
    try {
        ...
        // 更新装备
        const success = updateEquipment('current_user', props.equipment.id, {
            name: form.name.trim(),
